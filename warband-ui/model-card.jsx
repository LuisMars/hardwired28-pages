...
const StatRow = ({ model, onChange, mode, archetypes }) => {
  ...
  const StatCell = ({ stat, iconName, label }) => {
    ...
    const totalMod = modFromModifiers + buildMod;
    const final = applyDT(base, totalMod);
    const isPlay = mode === 'play';

    return (
      <div className={`flex-1 text-center ${isPlay ? 'p-1 sm:p-2' : 'p-2'} border-r border-[#333] last:border-r-0`}>
        ...
        {mode === 'build' && (
          <div className="flex items-center justify-center gap-1 mt-1">
            <button
              ...
              className="w-6 h-6 rounded border border-[#333] text-sm font-bold disabled:opacity-30 hover:bg-[#f5f5f0]"
            >-</button>
            <span className={`text-xs w-6 ${buildMod === 0 ? 'text-[#666]' : buildMod > 0 ? 'text-[#c9a227]' : 'text-[#8b2500]'}`}>
              {buildMod > 0 ? '+' : ''}{buildMod}
            </span>
            <button
              onClick={() => onChange({ ...model, [`${stat}Mod`]: buildMod + 1 })}
        ...
        {mode !== 'build' && totalMod !== 0 && (
          <div className="text-xs text-[#666]">
            ...
            <span className={totalMod > 0 ? 'text-[#c9a227] ml-1' : 'text-[#8b2500] ml-1'}>
              ({totalMod > 0 ? '+' : ''}{totalMod})
            </span>
          </div>
        )}
  ...
  return (
    <div className="border-2 border-[#333] rounded bg-white mb-2">
      {mode === 'build' && (
        <div className="flex gap-2 p-2 border-b border-[#333] bg-[#f5f5f0] text-xs">
          <select
            value={model.statArray || 'Operative'}
            onChange={(e) => onChange({ ...model, statArray: e.target.value })}
            className="border border-[#333] rounded px-2 py-1 flex-1 bg-white"
          >
          ...
          {model.statArray === 'Technician' && (
            <>
              <select
                ...
                className="border border-[#333] rounded px-2 py-1 bg-white"
              >
...
const HealthMoveRow = ({ model, onChange, mode }) => {
  ...
  const totalMoveMod = moveMod + armorMoveMod;
  const move = baseMove + totalMoveMod;
  const isPlay = mode === 'play';

  return (
    <div className="flex gap-2 mb-2">
      <div className="flex-1 border-2 border-[#333] rounded p-2 bg-white">
        <div className="flex items-center justify-between mb-1">
          ...
          <span className="text-xs text-[#666]">
            ...
            {healthMod !== 0 && (
              <span className={healthMod > 0 ? 'text-[#c9a227] ml-1' : 'text-[#8b2500] ml-1'}>
                ({t('base')} {baseHealth})
              </span>
            )}
          </span>
      ...
      <div className={`${isPlay ? 'w-20 sm:w-24' : 'w-24'} border-2 border-[#333] rounded p-2 bg-white text-center`}>
        ...
        {totalMoveMod !== 0 && (
          <div className={`text-xs ${totalMoveMod > 0 ? 'text-[#c9a227]' : 'text-[#8b2500]'}`}>
            ({t('base')} {baseMove}")
          </div>
        )}
      </div>
...
const ItemBadge = ({ item, onRemove, onToStash, onUse, used, showActions = true, damageBonus = 0 }) => {
  ...
  return (
    <div className={`flex items-center gap-2 p-2 rounded border border-[#333] text-xs group ${
      used ? 'bg-[#f5f5f0] opacity-50' : 'bg-white'
    }`}>
      {item.type && <TypeLabel type={item.type} />}
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-1">
          <span className={`font-semibold ${used ? 'line-through' : ''}`}>{item.name}</span>
          {item.free && <span className="text-[#c9a227] text-xs">({t('free')})</span>}
        </div>
        <div className="text-[#666] truncate text-xs">
          {item.damage && (
            <span className="mr-1">
              {hasBonus ? (
                <>
                  <span className="font-mono text-[#c9a227] font-bold">{effectiveDamage}</span>
                  <span className="text-[#999] ml-1">({item.damage}+{damageBonus})</span>
                </>
              ) : (
      ...
      {showActions && (
        <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
          {isCons && !used && onUse && (
            <button onClick={onUse} className="px-2 py-1 bg-[#1a1a1a] text-white rounded font-bold hover:bg-[#333]">
              {t('use')}
            </button>
          )}
          {onToStash && !used && (
            <button onClick={onToStash} className="px-1.5 py-1 border border-[#333] rounded hover:bg-[#f5f5f0]" title={t('toStash')}>
              <Icon name="stash" />
            </button>
          )}
          {onRemove && (
            <button onClick={onRemove} className="px-1.5 py-1 border border-[#333] text-[#8b2500] rounded hover:bg-[#f5f5f0]">
              <Icon name="remove" />
            </button>
          )}
        </div>
...
const EquipmentSection = ({ model, onChange, onAddToStash, mode, subs, onBuy, ascendedBonus, archetypes, weaponData }) => {
  ...
  const ShopModal = ({ items, title, onClose }) => (
    <div className="absolute z-50 top-0 left-0 right-0 bg-white border-2 border-[#333] rounded shadow-xl max-h-80 overflow-hidden">
      <div className="flex items-center justify-between p-2 bg-[#1a1a1a] text-white border-b border-[#333] sticky top-0">
        <span className="font-bold text-sm">{title}</span>
        <button onClick={onClose} className="text-[#999] hover:text-white text-lg">
          <Icon name="remove" />
        </button>
      </div>
      <div className="overflow-y-auto max-h-64">
        {items.map((item, idx) => (
          <div key={item.id || item.name || idx} className="p-2 border-b border-[#eee] hover:bg-[#f5f5f0] text-xs">
            <div className="flex items-center gap-2 mb-1">
              {item.type && <TypeLabel type={item.type} />}
              <span className="font-bold">{item.name}</span>
              {item.damage && <span className="font-mono text-[#666]">{item.damage}</span>}
              {item.reduction && <span className="text-[#666]">-{item.reduction}</span>}
              {item.tier && <span className="text-[#666]">({t('tier' + item.tier)})</span>}
              <div className="flex-1" />
              <button
                onClick={() => addItem(item, true)}
                className="px-2 py-1 border border-[#333] rounded hover:bg-[#f5f5f0] font-medium"
              >
                {t('free')}
              </button>
              <button
                ...
                className="px-2 py-1 bg-[#c9a227] text-black rounded hover:bg-[#8b6914] disabled:opacity-40 font-medium"
              >
                {item.cost}$
              </button>
            </div>
  ...
  const SectionHeader = ({ iconName, label, onAdd, addLabel }) => (
    <div className="flex items-center justify-between mb-1">
      ...
      {canEdit && onAdd && (
        <button onClick={onAdd} className="text-xs px-2 py-0.5 border border-[#333] rounded hover:bg-[#f5f5f0]">
          + {addLabel || t('add')}
        </button>
      )}
    </div>
    ...
    <div className="space-y-2 relative">
      ...
      <div>
        <div className="flex items-center justify-between mb-1">
          <span className="text-xs font-semibold text-[#666]">
            ...
            {rangedDmgBonus > 0 && <span className="text-[#c9a227] ml-1">(+{rangedDmgBonus} {t('ranged').toLowerCase()})</span>}
          </span>
          {canEdit && !(canMelee && canRanged) && (
            <button
              onClick={() => setShopOpen(canMelee ? 'melee' : 'ranged')}
              className="text-xs px-2 py-0.5 border border-[#333] rounded hover:bg-[#f5f5f0]"
            >
              + {t('add')}
            </button>
          )}
        ...
        {canEdit && canMelee && canRanged && (
          <div className="flex gap-1 mb-1">
            <button onClick={() => setShopOpen('melee')} className="text-xs px-2 py-0.5 border border-[#333] rounded hover:bg-[#f5f5f0]">+ {t('melee')}</button>
            <button onClick={() => setShopOpen('ranged')} className="text-xs px-2 py-0.5 border border-[#333] rounded hover:bg-[#f5f5f0]">+ {t('ranged')}</button>
          </div>
        )}
        <div className="space-y-1">
          ...
          {melee.length === 0 && ranged.length === 0 && (
            <div className="text-xs text-[#999] italic p-2 bg-[#f5f5f0] rounded border border-[#ddd]">{t('unarmed')}</div>
          )}
        </div>
      ...
      <div>
        ...
        {armor ? (
          ...
          <div className="text-xs text-[#999] italic p-2 bg-[#f5f5f0] rounded border border-[#ddd]">{t('noArmor')}</div>
        )}
...
const ModifiersSection = ({ model, onChange, mode }) => {
  ...
  return (
    <div className="border-2 border-[#333] rounded p-2 bg-white">
        ...
        {canEdit && (
          <div className="flex gap-1">
            <button
              ...
              className={`text-xs px-2 py-0.5 rounded border ${showAdd === 'negative' ? 'bg-[#8b2500] text-white border-[#8b2500]' : 'border-[#333] text-[#8b2500] hover:bg-[#f5f5f0]'}`}
            >
              - {t('negative')}
            </button>
            <button
              onClick={() => setShowAdd(showAdd === 'positive' ? null : 'positive')}
              className={`text-xs px-2 py-0.5 rounded border ${showAdd === 'positive' ? 'bg-[#c9a227] text-black border-[#c9a227]' : 'border-[#333] text-[#8b6914] hover:bg-[#f5f5f0]'}`}
            >
              + {t('positive')}
            </button>
          </div>
      ...
      {showAdd && (
        <div className="mb-2 p-2 bg-[#f5f5f0] rounded border border-[#ddd]">
            ...
            {MODIFIERS[showAdd].map(mod => (
              <button
                ...
                className={`px-2 py-1 rounded text-xs font-medium border ${
                  showAdd === 'negative' ? 'border-[#8b2500] text-[#8b2500] hover:bg-white' : 'border-[#8b6914] text-[#8b6914] hover:bg-white'
                }`}
              >
      ...
      {modifiers.length === 0 ? (
        <div className="text-xs text-[#999] italic text-center py-2">{t('noModifiers')}</div>
      ) : (
        <div className="flex flex-wrap gap-1">
          {modifiers.map((mod, i) => (
            <span
              ...
              className={`inline-flex items-center gap-1 px-2 py-1 rounded text-xs font-medium group border ${
                mod.id.includes('+') ? 'border-[#c9a227] text-[#8b6914]' : 'border-[#8b2500] text-[#8b2500]'
              }`}
            >
              <i className={mod.iconClass} /> {mod.name}
              {canEdit && (
                <button
                  ...
                  className="opacity-0 group-hover:opacity-100 ml-1 text-[#999] hover:text-[#333]"
                >
                  <Icon name="remove" />
                </button>
              )}
...
const ModelCard = ({ model, onChange, onFire, mode, subs, onSpend, onAddToStash, modelIndex, allModels, leaderDead, hasAscendedLeader, archetypes, weaponData, powers }) => {
  ...
  if (!model.archetype) {
    return (
      <div className="border-2 border-dashed border-[#999] rounded-lg bg-[#f5f5f0] p-6 flex flex-col items-center justify-center min-h-[200px]">
        <Icon name="add" className="text-4xl text-[#999] mb-3" />
        <span className="text-[#666] mb-3">{t('emptySlot')}</span>
        {mode === 'build' && (
          <select
            onChange={(e) => {
            ...
            className="text-sm border-2 border-[#333] rounded px-3 py-2 bg-white"
            defaultValue=""
          >
        ...
        {mode !== 'build' && (
          <span className="text-xs text-[#999]">{t('switchToBuild')}</span>
        )}
      </div>
    );
  }
  ...
  return (
    <div className={`border-2 rounded-lg bg-white overflow-hidden ${
      isLeader ? 'border-[#c9a227]' :
      model.ascended ? 'border-[#8b2500]' :
      'border-[#333]'
    }`}>
      {/* Header */}
      <div className={`px-3 py-2 border-b-2 ${
        isLeader ? 'bg-[#1a1a1a] text-[#c9a227] border-[#c9a227]' :
        model.ascended ? 'bg-[#8b2500] text-white border-[#8b2500]' :
        'bg-[#f5f5f0] border-[#333]'
      }`}>
        <div className="flex flex-wrap items-center gap-2">
          {isLeader && <Icon name="leader" className="text-[#c9a227]" />}
          {model.ascended && !isLeader && <Icon name="leader" className="text-[#f5f5f0]" title={t('makeLeader')} />}
          <input
            type="text"
            ...
            className={`flex-1 font-bold bg-transparent border-b border-dashed border-transparent hover:border-current focus:border-current focus:outline-none ${
              model.ascended && !isLeader ? 'text-white placeholder-[#ccc]' : ''
            }`}
          />
          <button
          ...
          {canEditArchetype ? (
            <select
              ...
              className="text-xs border border-[#333] rounded px-2 py-1 bg-white text-[#1a1a1a]"
            >
            ...
            <span className={`text-xs font-medium px-2 py-1 rounded ${
              model.ascended && !isLeader ? 'bg-[#f5f5f0] text-[#1a1a1a]' : 'bg-[#1a1a1a] text-[#f5f5f0]'
            }`}>
              {stripMarkdown(model.archetype) || t('noArchetype')}
            </span>
          )}
          {model.ascended && model.ascendedBonus && (
            <span className="text-xs px-2 py-1 rounded bg-white text-[#1a1a1a]">
          ...
          {mode !== 'play' && model.archetype && (
            <button
              onClick={onFire}
              className="text-xs px-2 py-1 border border-[#333] rounded hover:bg-white"
        ...
        {arch && (
          <div className={`text-xs mt-1 flex gap-2 flex-wrap items-center ${
            model.ascended && !isLeader ? 'text-[#f5f5f0]/80' : isLeader ? 'text-[#c9a227]/70' : 'text-[#666]'
          }`}>
            ...
            <span>{t('max')} {model.ascendedBonus === 'armor' ?
              ...
              (['Light', 'Ligera'].includes(arch.maxArmor) ? (arch.maxArmor === 'Light' ? 'Medium' : 'Media') :
               ...
               arch.maxArmor)
              : arch.maxArmor}</span>
            {arch.special !== '-' && <span className={model.ascended && !isLeader ? 'text-[#f5f5f0]' : 'text-[#8b2500]'} dangerouslySetInnerHTML={{ __html: markdownToHtml(arch.special) }} />}
            <span className={`font-bold px-2 py-0.5 rounded ml-auto ${
              isLeader ? 'bg-[#c9a227] text-[#1a1a1a]' : 'bg-[#1a1a1a] text-[#c9a227]'
            }`}>
              {equipmentCost}$
            </span>
          </div>
        ...
        {canAscend && mode === 'edit' && !showAscendChoice && (
          <button
            ...
            className="mt-2 w-full py-1 bg-[#8b2500] text-white text-xs font-bold rounded hover:bg-[#c73e1d]"
          >
            <Icon name="leader" className="mr-1" />{t('makeLeader')}
          </button>
        )}
        {showAscendChoice && (
          <div className="mt-2 p-2 bg-white rounded border border-[#333]">
            <div className="text-xs font-bold mb-2">{t('ascensionBonus')}</div>
            <div className="flex gap-2">
              <button
                onClick={() => handleAscend('weapons')}
                className="flex-1 py-2 bg-[#1a1a1a] text-white text-xs font-bold rounded hover:bg-[#333]"
              >
                {t('bothWeapons')}
              </button>
              <button
            ...
            <button
              ...
              className="mt-1 w-full text-xs text-[#666] hover:underline"
            >
              {t('cancel')}
            </button>
          </div>
      ...
      <div className="p-3 space-y-3">
        ...
        {isBroadcaster && (
          <div className="border-2 border-[#333] rounded p-2 bg-white">
            ...
            {model.power ? (
              <div className="flex items-center gap-2 p-2 bg-[#f5f5f0] rounded border border-[#ddd] text-xs">
                <span className="font-bold">{model.power.name}</span>
                <span className="text-[#666]" dangerouslySetInnerHTML={{ __html: 'vs ' + markdownToHtml(model.power.vs) }} />
                <span className="text-[#666] flex-1" dangerouslySetInnerHTML={{ __html: markdownToHtml(model.power.effect) }} />
                {(mode === 'build' || mode === 'edit') && (
                  <button
                    onClick={() => onChange({ ...model, power: null })}
                    className="text-[#8b2500] hover:text-[#c73e1d]"
                  >
                    <Icon name="remove" />
                  </button>
                )}
              ...
              <select
                ...
                className="w-full text-xs border border-[#333] rounded px-2 py-1 bg-white"
                disabled={mode === 'play'}
              >
        ...
        {/* Merits */}
        <div className="flex items-center gap-2 pt-2 border-t border-[#ddd]">
          <div className="flex items-center gap-1 border border-[#333] px-3 py-1 rounded">
            <span className="text-xs font-semibold">
            ...
            <input
              ...
              className="w-12 text-sm font-bold text-center bg-white border border-[#ddd] rounded"
              disabled={mode === 'play'}
            />
          </div>
          <span className="text-xs text-[#666]">
