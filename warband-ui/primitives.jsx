...
const TypeLabel = ({ type }) => {
  ...
  return (
    <span className="font-mono text-xs font-bold text-[#666] uppercase">{label}</span>
  );
};

// Mode selection tabs (Zorn palette)
const ModeSelector = ({ mode, onChange }) => (
  <div className="flex border-2 border-[#333] rounded overflow-hidden">
      ...
      return (
        <button
          ...
          className={`px-3 sm:px-4 py-2 sm:py-2 font-bold text-xs sm:text-sm transition-colors ${
            isActive
              ? `${style.bg} ${style.text}`
              : 'bg-white text-[#333] hover:bg-[#f5f5f0]'
          }`}
        >
          {style.label}
        </button>
